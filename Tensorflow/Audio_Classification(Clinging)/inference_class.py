...
class SoundClassificationService:
    ...
    def listen_and_predict(self, duration=0.4, overlap=0.5):
                ...
                while True:
                    try:
                        audio_chunk, _ = stream.read(
                            int(self.config["sample_rate"] * overlap)
                        )
                    except Exception as e:
                    ...
                    buffer[-len(audio_chunk):] = audio_chunk.flatten()[:len(buffer[-len(audio_chunk):])]
